...
export default function DataTable({ titleTbl, data, searchTbl, pagination, paginationConfig, styleCustom }) {
  ...
  if(styleCustom){
    Object.entries(styleCustom).map(([prop,value])=>{
      if(prop.includes('__')){
        ...
        if(key.includes('icon.color')||key.includes('icon.size')){
          eval(`style.${key} = " ${value}"`);
        }else{
          eval(`style.${key} += " ${value}"`); // add custom className
        }
      }else{
        eval(`style.${prop} += " ${value}"`);
      }
    })
  }
